...
const ViewRoute = (props) => {
    const { loadRouteData, routeId, avganger, ...rest } = props;

    console.log(`avganger=`, avganger);

    const [ avgangerLocal, setAvgangerLocal ] = useState(avganger);

    console.log(`avgangerLocal=`, avgangerLocal);

    useEffect(() => {
        setAvgangerLocal(avganger);
    }, [ avganger ]);

    useEffect(() => {
        const interval = setInterval(
            () => {
                setAvgangerLocal(state => {
                    const before = { avganger: state };
                    console.log(`removePassedAvganger before=`, before);
                    const result = removePassedAvganger(before);
                    console.log(`removePassedAvganger after=`, result);
                    return result.avganger;
                });
            },
            60000
        );
    ...
    return <ViewRouteInner
        ...
        routeId={routeId}
        {...rest}
        onRefresh={() => onRefresh(loadRouteData, routeId)}
    />;
};
